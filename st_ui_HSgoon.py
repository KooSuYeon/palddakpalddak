...
if not st.session_state['option_selected']:
    ...
    st.markdown(
        ...
        <div class="center-content">
            <h1>🤖팔딱팔딱 AI QUIZ🤖</h1>
            <h6>⬅️⬅️왼쪽에 있는 사이드바에서 원하는 주제와 교재를 선택해주세요.</h6>
            <img src="https://viralsolutions.net/wp-content/uploads/2019/06/shutterstock_749036344.jpg" alt="이미지 예시">
        </div>
        """,
        unsafe_allow_html=True,
    )
...
# 입력창과 버튼을 같은 줄에 배치
    col1, col2, col3, col4 = st.columns([0.3, 0.3, 0.1, 0.3])  # 입력창과 버튼의 비율 조정 (3:1)
    with col2:
        user_id = st.text_input("ID 입력", placeholder="ID 입력", label_visibility="collapsed", help="ID를 입력하세요")
    ...
    st.divider()
    st.text("8조 구성 멤버 - 구수연(팀장), 박성진, 이현승, 김윤소, 윤수진")
else:
    st.write(f"현재 세션의 ID: {st.session_state['user_id']}")
